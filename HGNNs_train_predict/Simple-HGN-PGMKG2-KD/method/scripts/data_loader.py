...
class data_loader:
    ...
    def evaluate(edge_list, confidence, labels):
        ...
        # 初始化评估指标列表
        mrr_list, mr_list = [], []
        hit_at_1_list, hit_at_3_list, hit_at_5_list, hit_at_10_list = [], [], [], []
        ...
        for h_id in t_dict.keys():
            ...
            # MR计算（排名从1开始）
            mr_list.append(pos_min_rank + 1)
            
            # Hit@K计算
            hit_at_1_list.append(1 if pos_min_rank < 1 else 0)
            hit_at_3_list.append(1 if pos_min_rank < 3 else 0)
            hit_at_5_list.append(1 if pos_min_rank < 5 else 0)
            hit_at_10_list.append(1 if pos_min_rank < 10 else 0)
        
        # 计算平均值
        mrr = np.mean(mrr_list) if mrr_list else 0.0
        mr = np.mean(mr_list) if mr_list else 0.0
        hit_at_1 = np.mean(hit_at_1_list) if hit_at_1_list else 0.0
        hit_at_3 = np.mean(hit_at_3_list) if hit_at_3_list else 0.0
        hit_at_5 = np.mean(hit_at_5_list) if hit_at_5_list else 0.0
        hit_at_10 = np.mean(hit_at_10_list) if hit_at_10_list else 0.0
    ...
    def get_test_neigh_w_random(self):
        ...
        for r_id in edge_types:
            ...
            for h_id, t_id in zip(row, col):
                ...
                attempts = 0
                neg_t = random.randrange(t_range[0], t_range[1])
                while neg_t in all_had_neigh[h_id] and attempts < max_attempts:
                    neg_t = random.randrange(t_range[0], t_range[1])
                    attempts += 1
                
                # 如果尝试次数过多仍未找到，使用随机负样本
                if attempts >= max_attempts:
                    print(f"警告: 节点 {h_id} 难以找到负样本，使用随机采样")
                    neg_t = random.randrange(t_range[0], t_range[1])
