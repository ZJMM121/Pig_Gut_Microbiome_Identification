...
class Runner(object):
	...
	def predict(self, split='valid', mode='tail_batch'):
		...
		with torch.no_grad():
			...
			pbar = tqdm(enumerate(train_iter), 
					   desc=f'{split.title()} {mode}', 
					   total=len(self.data_iter['{}_{}'.format(split, mode.split('_')[0])]),
					   leave=False)
			...
			for step, batch in pbar:
				...
				pbar.set_postfix({'MRR': f'{current_mrr:.5f}'})
	...
	def run_epoch(self, epoch, val_mrr = 0):
		...
		pbar = tqdm(enumerate(train_iter), 
				   desc=f'Epoch {epoch}', 
				   total=len(self.data_iter['train']),
				   leave=False)
		...
		for step, batch in pbar:
			...
			pbar.set_postfix({
				...
				'Val MRR': f'{val_mrr:.5f}',
				'Best MRR': f'{self.best_val_mrr:.5f}'
			})

			if step % 100 == 0:
				self.logger.info('[E:{}| {}]: Train Loss:{:.5},  Val MRR:{:.5}\t{}'.format(epoch, step, current_loss, self.best_val_mrr, self.p.name))
	...
	def fit(self):
		...
		# 添加总体训练进度条
		epoch_pbar = tqdm(range(self.p.max_epochs), desc='Training Progress', unit='epoch')
